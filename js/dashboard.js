...
window.cargarDashboard = async function() {
  ...
  } catch (error) { console.error("Error resumen:", error); }
...
async function cargarGraficoVentasMes() { 
    try { 
        ...
        chartVentas = new Chart(ctx, { 
            type: 'line', 
            data: { labels: data.map(d => d.dia), datasets: [{ label: 'Ingresos C$', data: data.map(d => d.total_dia), borderColor: '#ff69b7', backgroundColor: 'rgba(255, 105, 183, 0.2)', borderWidth: 2, fill: true, tension: 0.3 }] }, 
...
async function cargarGraficoTopProductos() { 
    try { 
        ...
        chartTop = new Chart(ctx, { 
            type: 'doughnut', 
            data: { labels: data.map(d => d.nombre), datasets: [{ data: data.map(d => d.total_vendido), backgroundColor: ['#ff69b7', '#ff9f43', '#ffc107', '#28c76f', '#20c997'], borderWidth: 0 }] }, 
            options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom' } } } 
        }); 
    } catch (error) {} 
}

// ESTA ES LA FUNCIÓN DEL BOTÓN AMARILLO (AHORA SEGURA EN WINDOW)
window.abrirReporteCompleto = async function() {
  const modalEl = document.getElementById("modalReporteProductos");
  if(!modalEl) return alert("Falta el HTML del modal de reportes.");
  ...
  if(modalHeader && !document.getElementById("btn-exportar-pdf")) {
      ...
      btnPdf.onclick = window.generarPDFReporte; 
  ...
  if(tbody) tbody.innerHTML = "<tr><td colspan='4' class='text-center'>Cargando desglose...</td></tr>"; 
  if(divMensual) divMensual.innerHTML = "<div class='text-center p-4 text-muted'>Calculando...</div>";
  
  try { 
      const res = await fetch(`${API_URL_DASH}/reportes/financiero`); 
      const datos = await res.json(); 
      datosReporteGlobal = datos; 
      if(tbody) {
          tbody.innerHTML = ""; 
          if(datos.length === 0) { 
              tbody.innerHTML = "<tr><td colspan='4' class='text-center text-muted'>No hay registros.</td></tr>"; 
          } else { 
              datos.forEach(d => { tbody.insertAdjacentHTML('beforeend', `<tr><td class="fw-bold">${d.producto}</td><td class="text-center">${d.tickets}</td><td class="text-center fw-bold">${d.unidades}</td><td class="text-end fw-bold text-success">C$ ${d.ingreso_total}</td></tr>`); }); 
          } 
      }
  } catch (error) {}
  ...
  try { 
      ...
      datosMensualGlobal = datosM; 
      if(divMensual) {
          divMensual.innerHTML = ""; 
          if(datosM.length === 0) { 
              divMensual.innerHTML = "<div class='text-center text-muted p-4'>No hay facturas.</div>"; 
          } else { 
              let tablaHTML = `<table class="table table-hover table-bordered align-middle m-0"><thead class="table-dark"><tr><th>Período Contable</th><th class="text-center">Volumen</th><th class="text-end">Recaudado</th></tr></thead><tbody>`; 
              datosM.forEach(m => { tablaHTML += `<tr><td class="fw-bold text-primary">Mes de ${mesANombre(m.mes)}</td><td class="text-center">${m.total_tickets} facturas</td><td class="text-end fw-bold text-success">C$ ${m.total_ganado}.00</td></tr>`; }); 
              tablaHTML += `</tbody></table>`; divMensual.innerHTML = tablaHTML; 
          } 
      }
  } catch (error) {}
};
...
window.generarPDFReporte = function() {
    ...
    ventana.document.write('<html><body onload="setTimeout(function(){ window.print(); window.close(); }, 500);">'); 

...
function mostrarErrorLogin(mensaje) { const errorDiv = document.getElementById("login-error"); if(errorDiv) { errorDiv.innerText = mensaje; errorDiv.style.display = "block"; } }
...
document.getElementById("form-agregar-usuario")?.addEventListener("submit", async function (e) { e.preventDefault(); const nombre = document.getElementById("nombre-usuario").value.trim(); const cargo = document.getElementById("cargo-usuario").value.trim(); const contrasea = document.getElementById("contrasea-usuario").value.trim(); if (!nombre || !cargo || !contrasea) return mostrarNotificacion("Faltan datos", "Todos los campos son obligatorios.", "warning"); const hash = await hashPassword(contrasea); try { const respuesta = await fetch(`${API_URL_USUARIOS}/empleados`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre, cargo, contraseña: hash }) }); if (!respuesta.ok) throw new Error("Error"); mostrarNotificacion("Completado", "Nuevo usuario del sistema creado.", "success"); bootstrap.Modal.getInstance(document.getElementById("modalAgregarUsuario")).hide(); if (document.getElementById("seccion-usuarios").style.display === "block") cargarTablaUsuariosAdmin(); } catch (error) { mostrarNotificacion("Fallo", "El servidor denegó la creación.", "error"); } });
...
document.getElementById("form-editar-usuario")?.addEventListener("submit", async function(e) { e.preventDefault(); const id = document.getElementById("edit-id-usuario").value; const nombre = document.getElementById("edit-nombre-usuario").value.trim(); const cargo = document.getElementById("edit-cargo-usuario").value.trim(); const passRaw = document.getElementById("edit-contrasea-usuario").value.trim(); if (!nombre || !cargo) return mostrarNotificacion("Atención", "El nombre y rol son requeridos", "warning"); const bodyData = { nombre, cargo }; if (passRaw) bodyData.password = await hashPassword(passRaw); try { const res = await fetch(`${API_URL_USUARIOS}/empleados/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(bodyData) }); if(!res.ok) throw new Error(); mostrarNotificacion("Actualizado", "Modificaciones guardadas.", "success"); bootstrap.Modal.getInstance(document.getElementById("modalEditarUsuario")).hide(); if (usuarioActual && usuarioActual.id == id) { usuarioActual.nombre = nombre; usuarioActual.cargo = cargo; actualizarHeaderUsuario(usuarioActual); } cargarTablaUsuariosAdmin(); } catch(error) { mostrarNotificacion("Error", "No se pudo actualizar.", "error"); } });
...
function filtrarUsuariosRender(val = "") {
  let filtrados = listaEmpleadosAdmin.filter(emp => emp.nombre.toLowerCase().includes(val) || emp.cargo.toLowerCase().includes(val));
  if(!verInactivosUsu) filtrados = filtrados.filter(e => e.activo !== false && e.activo !== 0);
...
function renderizarTablaUsuarios(empleados) {
  ...
  empleados.forEach(emp => {
    ...
    let botones = "";
    if (esInactivo) {
        botones = `<button class="btn btn-sm btn-success fw-bold" onclick="reactivarUsuario(${emp.id})"><i class="bi bi-arrow-counterclockwise"></i> Restaurar</button>`;
    } else {
        botones = `<button class="btn btn-sm btn-info me-1 text-white" onclick="abrirEditarUsuario(${emp.id})" title="Editar"><i class="bi bi-pencil"></i></button>`;
        if (usuarioActual && emp.id === usuarioActual.id) { botones += `<button class="btn btn-sm btn-outline-secondary" disabled title="Tú"><i class="bi bi-person-fill"></i></button>`; } 
        else { botones += `<button class="btn btn-sm btn-danger" onclick="eliminarUsuario(${emp.id})" title="Eliminar Acceso"><i class="bi bi-trash"></i></button>`; }
    }

    tbody.insertAdjacentHTML("beforeend", `
      <tr style="${rowStyle}">
        <td class="fw-bold">${emp.id}</td>
        <td><div class="d-flex align-items-center gap-2"><i class="bi bi-person-circle fs-4 text-muted"></i> ${emp.nombre} ${esInactivo ? '<span class="badge bg-danger">Inactivo</span>' : ''}</div></td>
        <td>${badgeRole}</td>
...
window.eliminarUsuario = async function(id) {
  ...
  mostrarConfirmacion("⚠️ Vas a eliminar a este usuario permanentemente, ¿Estás seguro?", async () => {
      try { await fetch(`${API_URL_USUARIOS}/empleados/${id}`, { method: 'DELETE' }); mostrarNotificacion("Eliminado", "Se ha revocado el acceso.", "success"); cargarTablaUsuariosAdmin(); } 
...
window.reactivarUsuario = async function(id) {
    mostrarConfirmacion("¿Deseas devolverle el acceso al sistema a este empleado?", async () => {
        try { await fetch(`${API_URL_USUARIOS}/empleados/${id}/reactivar`, { method: 'PUT' }); mostrarNotificacion("Restaurado", "Permisos devueltos.", "success"); cargarTablaUsuariosAdmin(); } 
        catch (error) { mostrarNotificacion("Fallo", "Error al restaurar.", "error"); }
    });
};

// ... Recuperacion de contraseña (igual) ...
window.iniciarRecuperacion = function() { const idUsuario = document.getElementById("login-usuario").value; if (!idUsuario) return mostrarNotificacion("Pausa", "Selecciona tu nombre en la lista de arriba antes de recuperar.", "warning"); new bootstrap.Modal(document.getElementById("modalRecuperarPass")).show(); };
document.getElementById("form-recuperar-pass")?.addEventListener("submit", async function(e) { e.preventDefault(); const idAdmin = document.getElementById("login-usuario").value; const pinMaestro = document.getElementById("recup-pin-maestro").value; const nuevaClave = document.getElementById("recup-nueva-pass").value; const confirmarClave = document.getElementById("recup-confirmar-pass").value; if (nuevaClave !== confirmarClave) return mostrarNotificacion("Error", "Las nuevas contraseñas no son iguales.", "error"); if (pinMaestro !== "UNI-2026") return mostrarNotificacion("Bloqueado", "PIN Administrativo denegado.", "error"); try { const hashNuevo = await hashPassword(nuevaClave); const res = await fetch(`${API_URL_USUARIOS}/empleados/${idAdmin}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ password: hashNuevo, es_recuperacion: true }) }); if (res.ok) { mostrarNotificacion("Desbloqueado", "Tu nueva contraseña fue asignada.", "success"); bootstrap.Modal.getInstance(document.getElementById("modalRecuperarPass")).hide(); document.getElementById("form-recuperar-pass").reset(); } else throw new Error(); } catch (error) { mostrarNotificacion("Error", "Hubo un fallo con el servidor de la base de datos.", "error"); } });

...
async function agregarInsumo(event) { event.preventDefault(); const nombre = document.getElementById("nombre-insumo").value.trim(); const unidad = document.getElementById("unidad-insumo").value.trim(); const precio = parseFloat(document.getElementById("precio-insumo").value); const proveedor_id = document.getElementById("proveedor-insumo").value || null; if (!nombre || !unidad || isNaN(precio) || precio <= 0) return mostrarNotificacion("Atención", "Error", "warning"); try { await fetch(`${API_URL_INS}/insumos`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre, unidad, precio, proveedor_id: proveedor_id ? parseInt(proveedor_id) : null }) }); bootstrap.Modal.getOrCreateInstance(document.getElementById("modalAgregarInsumo")).hide(); document.getElementById("form-agregar-insumo").reset(); mostrarNotificacion("OK", "Guardado", "success"); cargarInsumos(); } catch (error) {} }
async function abrirEditarInsumo(id) { try { const res = await fetch(`${API_URL_INS}/insumos/${id}`); const data = await res.json(); await cargarProveedoresSelect("edit-proveedor-insumo", data.proveedor_id); document.getElementById("edit-id-insumo").value = data.id; document.getElementById("edit-nombre-insumo").value = data.nombre; document.getElementById("edit-unidad-insumo").value = data.unidad || ""; document.getElementById("edit-precio-insumo").value = data.precio !== null ? data.precio : ""; bootstrap.Modal.getOrCreateInstance(document.getElementById("modalEditarInsumo")).show(); } catch (error) {} }
async function actualizarInsumo(event) { event.preventDefault(); const id = document.getElementById("edit-id-insumo").value; const nombre = document.getElementById("edit-nombre-insumo").value.trim(); const unidad = document.getElementById("edit-unidad-insumo").value.trim(); const precio = parseFloat(document.getElementById("edit-precio-insumo").value); const proveedor_id = document.getElementById("edit-proveedor-insumo").value || null; if (isNaN(precio) || precio <= 0) return mostrarNotificacion("Atención", "Inválido", "warning"); try { await fetch(`${API_URL_INS}/insumos/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nombre, unidad, precio, proveedor_id: proveedor_id ? parseInt(proveedor_id) : null }) }); bootstrap.Modal.getOrCreateInstance(document.getElementById("modalEditarInsumo")).hide(); mostrarNotificacion("OK", "Actualizado", "success"); cargarInsumos(); } catch (error) {} }

// ==========================================
// AQUI ESTÁN LAS FUNCIONES RESTAURADAS
// ==========================================
window.abrirCalculadora = function() { 
    bootstrap.Modal.getOrCreateInstance(document.getElementById("modalCalculadora")).show(); 
};
